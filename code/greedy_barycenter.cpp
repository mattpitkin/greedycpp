...
// Matt Pitkin (2017)
//
// The Greedy() function has been changed so that
// rather than adding a new basis vector based on the worst projection error
// with the current reduced basis set, it uses the worst residual error
// between the training waveforms and an empirical interpolant of the
// waveform produced using the current reduced basis. This requires that
// the input training set is not normalised (hence also a change from
// my_models.h to my_models_barycenter.h, which does not perform the
// normalisation. The original projection errors are also still computed.
//
// This has only been implemented for the OpenMP and standard complition
// of the code, and not for the MPI version
...
//          dim_RB: number of greedy_points
  ...
  gsl_matrix_complex *RB_space, *R_matrix, *RB_space_copy;
  ...
  int *eimind = NULL; // EIM indices
  gsl_matrix_complex *eiminvV; // EIM inverse matrix
  ...
  #pragma omp parallel private(projection_coeff)
  {
    ...
    while(continue_work)
    {
      ...
      #pragma omp master
      {
        // calculate EIM from current basis to use to get interpolation errors
        eim = new EIM(dim_RB, cols, false);

        // create RB_space copy (as build_eim_data modifies RB_space)
        gsl_matrix_complex_view RB_part = gsl_matrix_complex_submatrix(RB_space, 0, 0, dim_RB, cols);
        RB_space_copy = gsl_matrix_complex_alloc(dim_RB, cols);
        gsl_matrix_complex_memcpy(RB_space_copy, &RB_part.matrix);
        eim->build_eim_data(RB_space_copy);
        gsl_matrix_complex_free(RB_space_copy);
        RB_space_copy = gsl_matrix_complex_alloc(dim_RB, cols); // create new copy for building V matrix
        gsl_matrix_complex_memcpy(RB_space_copy, &RB_part.matrix);
        eim->rebuild_vandermonde(RB_space);
        eim->compute_vandermonde_inverse();
        
        // get EIM for evaluation
        eiminvV = gsl_matrix_complex_alloc(eim->invV().size1, eim->invV().size2);
        gsl_matrix_complex_memcpy(eiminvV, &eim->invV());
        eimind = (int *)calloc(eim->eim_size(), sizeof(int));
        memcpy(eimind, eim->p(), sizeof(int)*eim->eim_size());
        eimV = new EIM(dim_RB, cols, eiminvV, eimind);
      }
      #pragma omp barrier
      ...
      for(int i = 0; i < rows; i++)
      {
        ...
        // get residuals comparing interpolated TS waveform to original TS
        gsl_vector_complex *eim_eval = eimV->eim_full_vector(ts_el_omp, RB_space_copy, dim_RB);
        gsl_vector_complex_sub(eim_eval, ts_el_omp); // residuals
      
        // get maximum absolute residual value (for barycenter model is only real so get real vector view)
      ...
      gsl_matrix_complex_free(RB_space_copy);
      gsl_matrix_complex_free(eiminvV);
      delete eim; // free EIM
      eim = NULL;
      delete eimV;
      eimV = NULL;
      ...
      FindMax2(errors_res,rows,worst_err,worst_app);
      greedy_points[dim_RB] = worst_app;
      greedy_err[dim_RB]    = worst_err;
      ...
      mygsl::NormalizeVector(ortho_basis,wQuad); // normalise vector as training set not normalised in my_models_barycenter.h
      mygsl::IMGS(ru,ortho_basis,RB_space,wQuad,dim_RB); // IMGS is default
      //mygsl::MGS(ru,ortho_basis,RB_space,wQuad,dim_RB);
      gsl_matrix_complex_set_row(RB_space,dim_RB,ortho_basis);
      gsl_matrix_complex_set_row(R_matrix,dim_RB,ru);
  ...
  while(continue_work)
  {
    ...
    RB_space_copy = gsl_matrix_complex_alloc(dim_RB, cols); // create new copy for building V matrix
    gsl_matrix_complex_memcpy(RB_space_copy, &RB_part.matrix);
    eim->rebuild_vandermonde(RB_space_copy);
    eim->compute_vandermonde_inverse();
    ...
    for(int i = 0; i < rows; i++)
    {
      ...
      // get residuals comparing interpolated TS waveform to original TS
      gsl_vector_complex *eim_eval = eimV->eim_full_vector(ts_el, RB_space_copy, dim_RB);
      gsl_vector_complex_sub(eim_eval, ts_el); // residuals
